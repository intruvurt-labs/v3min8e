...
export default function CyberpunkChat() {
    ...
    if (screenProtection) {
      const preventScreenshot = (e: KeyboardEvent) => {
        ...
        if (
          ...
          e.preventDefault();
          addSystemMessage("🛡️ SCREENSHOT_BLOCKED: Screen capture prevented", "alert");
        }
      };

      const preventContext = (e: MouseEvent) => {
        e.preventDefault();
        addSystemMessage("🛡️ RIGHT_CLICK_BLOCKED: Context menu disabled for security", "alert");
      };
  ...
  useEffect(() => {
    setOnlineUsers([
      { id: "1", username: "DataGhost_01", role: "admin", lastSeen: Date.now() },
      { id: "2", username: "CryptoScanner", role: "premium", lastSeen: Date.now() - 30000 },
      { id: "3", username: "BlockchainNinja", role: "scanner", lastSeen: Date.now() - 120000 },
      { id: "4", username: "Anonymous_Ghost", role: "ghost", lastSeen: Date.now() - 60000 },
    ]);
  ...
  const scanAddress = async (address: string) => {
    ...
    try {
      ...
      const findings = [];
      if (riskLevel === "danger") {
        findings.push("Potential rug pull pattern detected", "High-risk transactions identified");
      } else if (riskLevel === "warning") {
        findings.push("Unusual trading patterns", "Medium risk indicators present");
      } else {
        findings.push("No major red flags detected", "Standard transaction patterns");
      }
      ...
      addSystemMessage("❌ SCAN_ERROR: Unable to complete address scan", "alert");
    } finally {
      setIsScanning(false);
    }
  };
  ...
  const sendMessage = async () => {
    ...
    if (detectedAddress) {
      addSystemMessage(`🎯 ADDRESS_DETECTED: Initiating scan on ${detectedAddress.substring(0, 8)}...`, "system");
      ...
      <div className="relative z-10 border-b border-cyber-green/30 p-4 bg-gradient-to-r from-dark-bg via-dark-bg/80 to-dark-bg">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {screenProtection ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            </button>
      ...
      <div className="flex h-96">
                    ...
                    {msg.scanResult && (
                      <div className="mt-3 p-3 bg-dark-bg/50 rounded border border-cyber-blue/20">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs font-cyber text-cyber-blue">SCAN_RESULT</span>
                        ...
                        <div className="text-xs space-y-1">
                          <div>Risk Score: <span className="font-bold">{msg.scanResult.riskScore}/100</span></div>
            ...
            {isScanning && (
              <div className="text-center">
                <div className="inline-flex items-center gap-2 bg-cyber-blue/10 border border-cyber-blue/30 rounded-lg p-3">
                  ...
                  <span className="text-cyber-blue font-mono text-sm">SCANNING_ADDRESS...</span>
                </div>
              </div>
            )}
          ...
          <div className="p-4 border-t border-cyber-green/30">
            <div className="flex gap-2">
              <input
                ...
                placeholder={connected ? "Type message or paste address to scan..." : "Connect wallet to chat..."}
                disabled={!connected}
        ...
        <div className="w-48 border-l border-cyber-green/30 bg-dark-bg/50 p-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-sm font-cyber font-bold text-cyber-green">ONLINE_GHOSTS</span>
          </div>
              ...
              <div key={user.id} className="flex items-center gap-2 p-2 rounded bg-dark-bg/30 border border-cyber-green/20">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-400">
                    {Date.now() - user.lastSeen < 60000 ? "online" : 
                     Date.now() - user.lastSeen < 300000 ? "away" : "offline"}
                  </div>
                </div>
              </div>
            ))}
